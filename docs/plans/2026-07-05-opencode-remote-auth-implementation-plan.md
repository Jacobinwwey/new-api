...
| Latest remote rollout | Done | Pushed `main` commit `c95d3c0d` is now deployed on the remote service. HTTP smoke returns 200, empty-state sidecar status returns successful `stopped`, remote Node sidecar retry tests pass, and the official OpenCode authorization page lifecycle smoke passed without credentials. |
...
| 最新远端上线 | 已完成 | 已推送的 `main` 提交 `c95d3c0d` 现在已部署到远端服务。HTTP smoke 返回 200，空 state 的 sidecar status 返回成功的 `stopped`，远端 Node sidecar retry 测试通过，官方 OpenCode 授权页无凭证 lifecycle smoke 通过。 |
