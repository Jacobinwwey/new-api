...
| `glm-5.2` cache-hit smoke runner | Implemented locally | Added `scripts/glm-cache-smoke.mjs` plus Node tests. The runner keeps API key, optional admin token/cookie, and optional cache key in environment variables; output includes only the model, rule/group, request counters, usage totals, channel-affinity stats, and the 8-character `key_fp`. If Admin auth is missing, relay calls still run and stats are marked skipped. HTTP 200 business failures with `success:false` are treated as failures and redacted before reporting, so auth/config failures are not miscounted as cache misses. Admin stats now use baseline/final snapshots plus `stats.delta` for the current run, with non-negative clamping and `reset_detected` when counters rotate or reset. This makes the future live E2E repeatable without committing or printing real credentials. |
...
node scripts/glm-cache-smoke.mjs --requests 6 --delay-ms 1000
```

7. Compare warm-cache behavior through the runner summary, New API channel-affinity usage stats, and upstream/OpenCode quota/accounting. For this smoke, treat `stats.delta` as the current-run evidence and `stats.data` as the final accumulated snapshot; if `reset_detected` is true, repeat the run after the cache window stabilizes. The runner proves request and stats plumbing; it does not by itself prove upstream prompt-cache billing correctness.
...
- `scripts/glm-cache-smoke.mjs` 现在提供一个可重复、输出脱敏的 cache-hit smoke runner，用于真实 E2E 阶段。它只从环境变量读取 relay/admin 凭据，多轮调用 `/v1/responses` 时使用稳定的 `prompt_cache_key`/`Session_id`，并可按非敏感 key fingerprint 查询 `/api/log/channel_affinity_usage_cache`。当 admin stats 可用时，runner 会记录 baseline 与 final 两个 usage-cache 快照，并输出本轮 `stats.delta`，避免把历史累计 counter 误当作本轮 cache-hit 证据。
...
| `glm-5.2` cache-hit smoke runner | 本地已实现 | 已新增 `scripts/glm-cache-smoke.mjs` 及 Node 测试。runner 会把 API key、可选 admin token/cookie 与可选 cache key 限定在环境变量中；输出只包含 model、rule/group、请求计数、usage 汇总、channel-affinity stats 与 8 位 `key_fp`。如果缺少 Admin auth，relay 请求仍会执行，stats 会标记为 skipped。HTTP 200 但 `success:false` 的业务失败会被当作失败并先脱敏再报告，因此 auth/config 失败不会被误算成 cache miss。Admin stats 现在使用 baseline/final 快照与本轮 `stats.delta`，并对 counter 轮转或重置做非负钳制与 `reset_detected` 标记。这样后续真实 E2E 可以重复执行，同时不提交或打印真实凭据。 |
...
7. 结合 runner 摘要、New API channel-affinity usage stats 与上游/OpenCode quota/accounting 比较 warm-cache 行为。本轮 smoke 应把 `stats.delta` 作为当前运行证据，`stats.data` 只作为 final 累计快照；如果 `reset_detected` 为 true，应等 cache 窗口稳定后重跑。runner 证明请求链路与统计读取链路可重复，不单独证明上游 prompt-cache 计费正确。
