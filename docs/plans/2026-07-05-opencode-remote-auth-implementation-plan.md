...
#### Current Main Snapshot (code baseline: `1c663ce9`)
...
The current update turns the fixed preflight readiness categories into a live E2E acceptance gate. `scripts/opencode-e2e-preflight.mjs` still reports fixed-category account summaries for credential key source and activation contract state, and `scripts/opencode-live-e2e.mjs` now requires `accounts.activation_contract.ready` to meet `minActiveReadyAccounts` before it can run `glm_cache_smoke`. This catches contradictory evidence such as `active_ready > 0` while no account is actually classified as `ready`, without printing account labels, channel names, masked emails, raw missing-field names, API keys, cookies, OAuth tokens, workspace IDs, or deployment addresses.
...
| Last verified remote rollout | Done | Pushed `main` commit `1c663ce9` is the last rollout verified on the remote service. The rollout used the clean rollout helper, first passed verification-only `git_clone`, `revision`, Node script, targeted Go, default-web check/build, classic-web build, Go build, and artifact gates, then passed the full apply path with service contract validation, backup, install, restart, HTTP smoke, and the default auth runtime smoke gate. Independent post-apply checks confirmed the service is active, local HTTP status is OK, six installed runtime scripts pass `node --check`, and the deployed OpenCode live E2E script contains the new `opencode_activation_contract_ready` gate. The temporary helper used for rollout was removed after verification. |
...
Latest remote rollout verification for pushed commit `1c663ce9`:
...
clean rollout apply:
  ...
  deployed_prefix=1c663ce9

post-apply smoke:
  service_active=ok
  http_smoke=ok
  runtime scripts node --check=ok
  live_e2e_activation_contract_gate_marker=ok
  cleanup_ok
```
...
本次更新把固定 preflight readiness 类别提升为 live E2E 验收 gate。`scripts/opencode-e2e-preflight.mjs` 仍然为 credential key source 与 activation contract state 输出固定类别汇总，而 `scripts/opencode-live-e2e.mjs` 现在会要求 `accounts.activation_contract.ready` 达到 `minActiveReadyAccounts` 后才允许运行 `glm_cache_smoke`。这能捕获 `active_ready > 0` 但没有账号真正被分类为 `ready` 的矛盾证据，同时不打印账号 label、channel 名、masked email、原始 missing-field 名、API key、cookie、OAuth token、workspace ID 或部署地址。
...
| 上一次已验证远端上线 | 已完成 | 已推送的 `main` 提交 `1c663ce9` 是当前在远端服务完成验证的上线版本。本次上线使用 clean rollout helper，先通过 verification-only `git_clone`、`revision`、Node script、定向 Go、default-web check/build、classic-web build、Go build 与 artifact gate，再通过完整 apply 路径中的 service contract 校验、backup、install、restart、HTTP smoke 与默认 auth runtime smoke gate。apply 后独立检查确认服务 active、本机 HTTP status OK，6 个已安装 runtime scripts 全部通过 `node --check`，且部署态 OpenCode live E2E 脚本包含新的 `opencode_activation_contract_ready` gate。本次使用的临时 helper 已在验证后清理。 |
...
已推送提交 `1c663ce9` 的最新远端 rollout 验证：
