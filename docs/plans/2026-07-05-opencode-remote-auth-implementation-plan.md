...
#### Current Main Snapshot (code baseline: `c60a4dc0`)
...
| Make rollout repeatable and secret-clean | Satisfied for artifact rollout. The clean rollout helper builds from a clean checkout, runs targeted Node/Go/frontend/build gates, redacts public output, installs only non-secret runtime scripts including the auth-session runtime smoke runner, and applies only with explicit `--apply true`. Commit `c60a4dc0` promotes the auth runtime smoke from a manual post-apply check into the default apply gate after restart and HTTP smoke, with an explicit `--auth-runtime-smoke false` diagnostic override. Pushed `main` commit `243055ca` verified that the full apply path now includes `auth_runtime_smoke=ok` before reporting the deployed prefix. | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs`, `scripts/opencode-auth-session-smoke.mjs` | Rollout helper does not own SSH transport, sudo provisioning, real account import, or secret configuration. Those remain operator/deployment responsibilities. |
...
| Last verified remote rollout | Done | Pushed `main` commit `243055ca` is the last rollout verified on the remote service. The rollout used the clean rollout helper, first passed verification-only clone/revision/Node/Go including the cross-package Go gate/OpenCode frontend/default frontend/classic frontend/Go build/artifact gates, then passed the full apply path with service contract validation, backup, install, restart, HTTP smoke, and the default auth runtime smoke gate. Independent post-apply checks confirmed the service is active, local HTTP `/api/status` is OK, and all installed runtime scripts pass `node --check`. |
...
| rollout 可重复且不携带 secret | artifact rollout 层面已满足。clean rollout helper 从干净 checkout 构建，执行定向 Node/Go/前端/build gate，脱敏公开输出，只安装非 secret runtime scripts，包括 auth-session runtime smoke runner，且只有显式 `--apply true` 才切换运行 artifact。提交 `c60a4dc0` 已把 auth runtime smoke 从人工 post-apply 检查提升为 restart 与 HTTP smoke 之后的默认 apply gate，并保留显式 `--auth-runtime-smoke false` 诊断开关。已推送的 `main` 提交 `243055ca` 验证完整 apply 路径现在会在 reported deployed prefix 前输出 `auth_runtime_smoke=ok`。 | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs`, `scripts/opencode-auth-session-smoke.mjs` | rollout helper 不负责 SSH 传输、sudo provisioning、真实账号导入或 secret 配置；这些仍是操作者/部署责任。 |
...
| 上一次已验证远端上线 | 已完成 | 已推送的 `main` 提交 `243055ca` 是当前在远端服务完成验证的上线版本。本次上线使用 clean rollout helper，先通过 verification-only clone/revision/Node/Go（包含跨包 Go gate）/OpenCode 前端/default 前端/classic 前端/Go build/artifact gate，再通过完整 apply 路径中的 service contract 校验、backup、install、restart、HTTP smoke 与默认 auth runtime smoke gate。apply 后独立检查确认服务 active、本机 HTTP `/api/status` OK，已安装的 runtime scripts 全部通过 `node --check`。 |
