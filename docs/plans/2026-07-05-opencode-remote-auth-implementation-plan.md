...
| Make rollout repeatable and secret-clean | Satisfied for artifact rollout. The clean rollout helper builds from a clean checkout, runs targeted Node/Go/frontend/build gates, redacts public output, installs only non-secret runtime scripts including the auth-session runtime smoke runner, and applies only with explicit `--apply true`. Commit `c60a4dc0` promotes the auth runtime smoke from a manual post-apply check into the default apply gate after restart and HTTP smoke, with an explicit `--auth-runtime-smoke false` diagnostic override. | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs`, `scripts/opencode-auth-session-smoke.mjs` | Rollout helper does not own SSH transport, sudo provisioning, real account import, or secret configuration. Those remain operator/deployment responsibilities. |
...
| Last verified remote rollout | Done | Pushed `main` commit `0dbd7964` is the last rollout verified on the remote service. The rollout used the clean rollout helper, first passed verification-only clone/revision/Node/Go including the cross-package Go gate/OpenCode frontend/default frontend/classic frontend/Go build/artifact gates, then passed the full apply path with service contract validation, backup, install, restart, and HTTP smoke. Independent post-apply checks confirmed the service is active, local HTTP `/api/status` is OK, all installed runtime scripts pass `node --check`, and the installed auth-session runtime smoke returns a successful JSON summary with start/status/press/screenshot/stop checks all true. |
...
| rollout 可重复且不携带 secret | artifact rollout 层面已满足。clean rollout helper 从干净 checkout 构建，执行定向 Node/Go/前端/build gate，脱敏公开输出，只安装非 secret runtime scripts，包括 auth-session runtime smoke runner，且只有显式 `--apply true` 才切换运行 artifact。提交 `c60a4dc0` 已把 auth runtime smoke 从人工 post-apply 检查提升为 restart 与 HTTP smoke 之后的默认 apply gate，并保留显式 `--auth-runtime-smoke false` 诊断开关。 | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs`, `scripts/opencode-auth-session-smoke.mjs` | rollout helper 不负责 SSH 传输、sudo provisioning、真实账号导入或 secret 配置；这些仍是操作者/部署责任。 |
...
| Auth sidecar runtime smoke gate | 本地已实现；runner 已完成远端 apply | 已新增 `scripts/opencode-auth-session-smoke.mjs`。该无凭证 runtime smoke 会让 sidecar 打开 `about:blank`，验证 running status，发送白名单 Escape 控制键，验证 PNG 截图输出，停止会话并 purge 浏览器 artifact。报告只暴露固定状态类别和布尔检查，不打印浏览器 URL/title、截图字节、state 路径、账号材料或部署 endpoint。clean rollout helper 会安装该 runner，并在 Node gate 中覆盖它的测试与语法检查。代码提交 `7aa5d64f` 已通过本地完整 Node rollout gate，共 89 个子测试；提交 `3e030e09` 修复了 runner 的 symlinked artifact 入口，因为第一次 post-apply 检查发现已安装脚本可能退出 0 但没有执行 `main()`。已推送的 `main` 提交 `0dbd7964` 通过远端 verification-only 与完整 apply；apply 后独立检查确认已安装 runner 返回成功 JSON 摘要，start/status/press/screenshot/stop 检查全部为 true。提交 `c60a4dc0` 现在把该 smoke 变成 clean rollout 默认 apply gate，而不再依赖人工 post-apply 步骤；该 helper 变更已有本地覆盖，等待远端 rollout。 |
...
| 上一次已验证远端上线 | 已完成 | 已推送的 `main` 提交 `0dbd7964` 是当前在远端服务完成验证的上线版本。本次上线使用 clean rollout helper，先通过 verification-only clone/revision/Node/Go（包含跨包 Go gate）/OpenCode 前端/default 前端/classic 前端/Go build/artifact gate，再通过完整 apply 路径中的 service contract 校验、backup、install、restart 与 HTTP smoke。apply 后独立检查确认服务 active、本机 HTTP `/api/status` OK，已安装的 runtime scripts 全部通过 `node --check`，且已安装的 auth-session runtime smoke 返回成功 JSON 摘要，start/status/press/screenshot/stop 检查全部为 true。 |
