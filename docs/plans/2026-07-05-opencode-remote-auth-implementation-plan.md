...
| Login status URL sanitization | Implemented locally | Status responses now strip URL credentials, query strings, and fragments from HTTP(S) browser URLs at the Node sidecar boundary before the value reaches Go, and the Go API layer keeps its own sanitization as a second boundary. This prevents OAuth `state`, `code`, bearer-like fragments, or URL userinfo from reaching sidecar CLI output, remote smoke logs, or the Admin UI/API response. |
...
The status response now treats browser URL as sensitive metadata at both process boundaries. OAuth redirect URLs can carry `state`, `code`, provider-specific query payloads, URL userinfo, or fragment tokens, so the Node sidecar strips HTTP(S) credentials, query strings, and fragments before emitting status JSON. The Go API layer keeps the same sanitization as defense in depth. The tradeoff is that status URLs are no longer full replay artifacts, but they still expose the navigational location needed by the operator without leaking authorization material into CLI output, smoke logs, or Admin responses.
...
| 登录状态 URL 脱敏 | 本地已实现 | status 响应现在会在 Node sidecar 边界移除 HTTP(S) 浏览器 URL 的 URL credentials、query string 与 fragment，再进入 Go 层；Go API 层仍保留自己的二次脱敏边界。这样 OAuth `state`、`code`、bearer-like fragment 或 URL userinfo 不会进入 sidecar CLI 输出、远端 smoke 日志或 Admin UI/API 响应。 |
...
状态响应现在在两个进程边界都把浏览器 URL 视为敏感元数据。OAuth redirect URL 可能携带 `state`、`code`、provider-specific query payload、URL userinfo 或 fragment token，因此 Node sidecar 会在输出 status JSON 前移除 HTTP(S) credentials、query string 与 fragment。Go API 层保留同样的脱敏作为 defense in depth。代价是 status URL 不再是完整可回放证据，但仍能暴露操作者需要看到的导航位置，同时避免授权材料进入 CLI 输出、smoke 日志或 Admin 响应。
