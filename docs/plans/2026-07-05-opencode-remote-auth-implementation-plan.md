...
| Auth sidecar runtime smoke gate | 本地已实现并完成候选远端 smoke | 已新增 `scripts/opencode-auth-session-smoke.mjs`。该无凭证 runtime smoke 会让 sidecar 打开 `about:blank`，验证 running status，发送白名单 Escape 控制键，验证 PNG 截图输出，停止会话并 purge 浏览器 artifact。报告只暴露固定状态类别和布尔检查，不打印浏览器 URL/title、截图字节、state 路径、账号材料或部署 endpoint。clean rollout helper 现在会安装该 runner，并在 Node gate 中覆盖它的测试与语法检查。代码提交 `7aa5d64f` 已通过本地完整 Node rollout gate，共 89 个子测试；提交 `3e030e09` 修复了 runner 的 symlinked artifact 入口，因为第一次 post-apply 检查发现已安装脚本可能退出 0 但没有执行 `main()`。临时远端候选运行先复现了已部署 sidecar 的固定 display Xvfb 启动失败，再使用更新后的 sidecar 通过 start/status/press/screenshot/stop；symlink 入口修复尚未完成正式远端 apply。 |
| Xvfb display 冲突重试 | 本地已实现并完成候选远端 smoke | sidecar 不再把 account-derived Xvfb display 当作单点。Linux 且没有既有 `DISPLAY` 时，`login/start` 会从账号派生 display 开始尝试一组有界候选，只对可重试的 `Xvfb exited before ready` 启动失败切换到下一个 display，并仍清理每次失败尝试已经拉起的 browser/Xvfb 进程。该改动修复了真实远端 runtime 中 display 冲突导致的启动失败，同时不会掩盖缺少 browser 或 Xvfb 依赖这类硬故障。回归测试锁定候选序列，临时远端候选 smoke 已通过新重试路径。代码提交 `7aa5d64f` 尚未通过 clean rollout helper 正式远端 apply。 |
