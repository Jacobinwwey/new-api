...
#### Current Main Snapshot (code baseline: `8c5d4e25`)
...
| Codex OAuth credential extraction | Implemented locally | The extractor now recognizes complete Codex OAuth credential objects from storage or same-site JSON responses. It accepts `access_token` only when paired with `account_id`, or when the account ID can be derived from the access-token JWT claim, then serializes the known fields into the existing Codex channel JSON key contract. Standalone OAuth token fields remain rejected as API-key candidates. Service tests cover extraction, JWT-derived account identity, and activation of a bound Codex channel with the extracted credential. |
...
| Codex OAuth 凭据提取 | 本地已实现 | extractor 现在能从 storage 或同站 JSON responses 中识别完整 Codex OAuth credential object。只有当 `access_token` 同时带有 `account_id`，或能从 access-token JWT claim 推出 account ID 时，才会把已知字段序列化为现有 Codex channel JSON key 契约；单独的 OAuth token 字段仍会被拒绝为 API-key 候选。服务层测试覆盖提取、JWT 推导账号身份，以及用提取到的凭据激活绑定的 Codex channel。 |
...
| 上一次已验证远端上线 | 已完成 | 已推送的 `main` 提交 `8c5d4e25` 是当前在远端服务完成验证的上线版本。本次上线使用 clean rollout helper，先通过 verification-only `git_clone`、`revision`、Node script、定向 Go、default-web check/build、classic-web build、Go build 与 artifact gate，再通过完整 apply 路径中的 service contract 校验、backup、install、restart、HTTP smoke 与默认 auth runtime smoke gate。apply 后独立检查确认服务 active、本机 HTTP status OK，6 个已安装 runtime scripts 全部通过 `node --check`。本次使用的临时 helper 已在验证后清理。 |
