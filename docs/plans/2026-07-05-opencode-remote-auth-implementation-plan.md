...
新的 live E2E runner 补齐了基础设施 readiness 与 cache-hit 证明之间的编排缺口。它刻意按 Tailscale link -> OpenCode 部署/账号 readiness -> `glm-5.2` cache smoke 的顺序执行 gate。这个顺序偏保守：当必须使用的 Tailscale 路径已经坏掉时，runner 不会继续消耗真实 relay/API 凭据，也不会产出一个不能代表目标生产路径的 cache-hit 结果。live gate 的验收语义是“所有实际执行的 stage 必须 passed”，而不是“没有显式 failed”：OpenCode readiness 或 cache smoke 意外返回 `skipped` 时应视为验收失败。代价是除非用 `--skip-tailscale true` 做本地诊断，否则 cache 测量会被网络正确性阻塞；这是正确的失败模式，因为用户需求明确要求 robust path 必须走 Tailscale。
