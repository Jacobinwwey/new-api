...
| Last verified remote rollout | Done | Pushed `main` commit `6e281c87` is the last rollout verified on the remote service. The rollout used the clean rollout helper, first passed verification-only clone/revision/Node/Go/OpenCode frontend/default frontend/classic frontend/Go build/artifact gates, then passed the full apply path with service contract validation, backup, install, restart, and HTTP smoke. Independent post-apply checks confirmed the service is active, local HTTP `/api/status` is OK, all installed runtime scripts pass `node --check`, and the auth sidecar returns the expected empty stopped status. |
...
| 上一次已验证远端上线 | 已完成 | 已推送的 `main` 提交 `6e281c87` 是当前在远端服务完成验证的上线版本。本次上线使用 clean rollout helper，先通过 verification-only clone/revision/Node/Go/OpenCode 前端/default 前端/classic 前端/Go build/artifact gate，再通过完整 apply 路径中的 service contract 校验、backup、install、restart 与 HTTP smoke。apply 后独立检查确认服务 active、本机 HTTP `/api/status` OK，已安装的 runtime scripts 全部通过 `node --check`，且 auth sidecar 返回预期的空状态 stopped。 |
