...
| OpenCode E2E preflight runner | Implemented locally | Added `scripts/opencode-e2e-preflight.mjs` plus Node tests. The runner performs non-mutating GET checks for `/api/status`, root-only OpenCode diagnostics, and OpenCode account readiness summary. It reads root auth only from environment variables, emits only endpoint status, credential key-source class, account counts, credential-integrity counts, and missing-field category counts, and exits non-zero when required root auth, stable credential key, or minimum activation-ready account counts are not satisfied. It also redacts the configured deployment URL from endpoint errors. This turns the first post-rollout verification step into a machine gate without printing root tokens, cookies, raw emails, workspace IDs, raw missing-field names, account secrets, deployment URLs, or local deployment paths. |
...
| OpenCode E2E preflight runner | 本地已实现 | 已新增 `scripts/opencode-e2e-preflight.mjs` 及 Node 测试。runner 会对 `/api/status`、root-only OpenCode diagnostics 与 OpenCode 账号 readiness 汇总执行非破坏性 GET 检查。它只从环境变量读取 root auth，输出只包含 endpoint 状态、credential key-source 类别、账号计数、credential-integrity 计数与 missing-field 类别计数；当 root auth、稳定 credential key 或最小 activation-ready 账号数不满足要求时非零退出。endpoint 错误中的部署 URL 也会被脱敏。这样远端 rollout 后的第一步验证可以变成机器门，同时不打印 root token、cookie、原始邮箱、workspace ID、原始 missing-field 名称、账号 secret、部署 URL 或本地部署路径。 |
