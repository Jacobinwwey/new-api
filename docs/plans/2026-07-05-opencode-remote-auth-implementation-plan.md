...
| Remote browser authorization from the deployed server | Satisfied for infrastructure and smoke-tested without credentials. The sidecar starts isolated Chromium/Xvfb sessions and exposes status, screenshot, click, stdin text input, safe control-key press, extract, stop, and purge actions. | `scripts/opencode-auth-session.mjs`, `service/opencode_browser_session.go`, `controller/opencode_account.go`, `web/default/src/features/opencode-accounts/api.ts` | Real Google/OpenCode authorization still requires operator-controlled subscription credentials and must remain outside the repository. If CDP interaction is insufficient in real login, add noVNC/streaming as a transport fallback without changing account or activation contracts. |
...
| Make rollout repeatable and secret-clean | Satisfied for artifact rollout. The clean rollout helper builds from a clean checkout, runs targeted Node/Go/frontend/build gates, redacts public output, installs only non-secret runtime scripts, and applies only with explicit `--apply true`. | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs` | Rollout helper does not own SSH transport, sudo provisioning, real account import, or secret configuration. Those remain operator/deployment responsibilities. |
...
| 在部署服务器上执行远端浏览器授权 | 基础设施已满足，并已完成无凭证 smoke。sidecar 可启动隔离 Chromium/Xvfb 会话，并提供 status、screenshot、click、stdin 文本输入、安全控制键 press、extract、stop、purge。 | `scripts/opencode-auth-session.mjs`, `service/opencode_browser_session.go`, `controller/opencode_account.go`, `web/default/src/features/opencode-accounts/api.ts` | 真实 Google/OpenCode 授权仍需要操作者控制的订阅账号，不能进入仓库。如果真实登录证明 CDP 操作不足，再增加 noVNC/streaming 作为传输兜底，不改变账号模型和激活契约。 |
...
| rollout 可重复且不携带 secret | artifact rollout 层面已满足。clean rollout helper 从干净 checkout 构建，执行定向 Node/Go/前端/build gate，脱敏公开输出，只安装非 secret runtime scripts，且只有显式 `--apply true` 才切换运行 artifact。 | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs` | rollout helper 不负责 SSH 传输、sudo provisioning、真实账号导入或 secret 配置；这些仍是操作者/部署责任。 |
