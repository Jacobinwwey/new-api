...
| 前端截图点击坐标准确性 | 本地已实现 | screenshot 点击坐标现在会在发送 CDP click 前扣除 `object-contain` 的 letterbox 留白。横向或纵向 padding 区域的点击会被忽略，内容边缘坐标会 clamp 到远端 viewport 内。该修复覆盖一个真实授权流程故障模式：响应式管理后台布局变化时，Google/OpenCode 页面点击可能落到错误的远端坐标。 |
