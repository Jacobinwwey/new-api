...
| Delete lifecycle cleanup | Implemented locally | Deleting an OpenCode account now stops the account's browser session before deleting the database row. If the stop action fails, the account is preserved so the operator can retry cleanup instead of losing the durable handle while a browser process may still exist. |
...
Account deletion now participates in the same lifecycle boundary. The controller stops the account's browser session before deleting the durable row; if stop fails, deletion fails closed and leaves the account available for retry. The tradeoff is that a broken sidecar can temporarily block deletion, but that is safer than deleting the only durable account handle while a remote browser process may still be running.
...
go test ./controller -run "TestDeleteOpenCodeAccountStopsLoginSessionBeforeDeleting|TestDeleteOpenCodeAccountPreservesAccountWhenStopFails" -count=1
...
| Delete 生命周期清理 | 本地已实现 | 删除 OpenCode 账号现在会先停止该账号的浏览器会话，再删除数据库记录。如果 stop 动作失败，账号会保留，操作者可以重试清理，避免在可能仍有浏览器进程存在时丢失持久化操作句柄。 |
...
账号删除现在也进入同一生命周期边界。controller 会在删除持久账号行前先停止该账号的浏览器会话；如果 stop 失败，删除会 fail closed，并保留账号以便重试。这里的取舍是：sidecar 故障可能暂时阻塞删除，但这比在远端浏览器进程可能仍运行时删除唯一的持久账号句柄更安全。
