...
| OpenCode frontend rollout gate | Implemented locally | The clean rollout helper now runs the OpenCode account page helper test, targeted oxlint for `src/features/opencode-accounts` and its route, and default-web typecheck before the default frontend build. This catches regressions in the operator-facing import window before a remote artifact switch, without treating unrelated whole-repo historical lint debt as an OpenCode rollout blocker. |
...
| Last verified remote rollout | Done | Pushed `main` commit `8afd80c5` is the last rollout verified on the remote service. The rollout used the clean rollout helper, first passed verification-only clone/revision/Node/Go/frontend/build/artifact gates, then passed the full apply path with service contract validation, backup, install, restart, and HTTP smoke. Independent post-apply checks confirmed the service is active, local HTTP `/api/status` is OK, and all installed runtime scripts pass `node --check`, including Tailscale preflight and the live E2E orchestrator. |
...
| OpenCode 前端 rollout gate | 本地已实现 | clean rollout helper 现在会在 default 前端构建前运行 OpenCode account 页面 helper 测试、针对 `src/features/opencode-accounts` 与其 route 的 targeted oxlint，以及 default-web typecheck。这样远端 artifact 切换前会覆盖操作者实际使用的导入窗口，同时不会把全仓既有历史 lint 债务误当成 OpenCode rollout 阻塞。 |
...
| 上一次已验证远端上线 | 已完成 | 已推送的 `main` 提交 `8afd80c5` 是当前在远端服务完成验证的上线版本。本次上线使用 clean rollout helper，先通过 verification-only clone/revision/Node/Go/前端/build/artifact gate，再通过完整 apply 路径中的 service contract 校验、backup、install、restart 与 HTTP smoke。apply 后独立检查确认服务 active、本机 HTTP `/api/status` OK，并且已安装的 runtime scripts 全部通过 `node --check`，包括 Tailscale preflight 与 live E2E orchestrator。 |
