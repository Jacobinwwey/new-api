...
POST   /api/opencode/accounts/:id/login/press
...
#### Current Main Snapshot (code baseline: `cdb13e4e`)
...
The current update separates production acceptance from diagnostic execution in the live E2E artifact. `scripts/opencode-live-e2e.mjs` now emits an `acceptance` summary with `production_ready`, `mode`, `diagnostic_overrides`, and `failed_checks`. A run can only be production-ready when all stage checks pass and no diagnostic override is active; `--skip-tailscale true` or `--continue-on-failure true` now produces `acceptance.status=failed` even when individual diagnostic stages complete. This prevents a local or partial diagnostic run from being mistaken for full Tailscale/OpenCode/cache acceptance, without printing account labels, channel names, masked emails, raw missing-field names, API keys, cookies, OAuth tokens, workspace IDs, deployment addresses, or raw network identifiers.

Follow-up commit `300ea33d` adds CLI-level regression coverage for the same boundary. A contract-complete fake New API server drives `opencode-live-e2e.mjs --skip-tailscale true`, verifies that stage checks can pass while `acceptance` remains diagnostic/failed, confirms the CLI exits with code `1`, checks that the cache-smoke stage still records measured hits, and asserts stdout omits relay/admin/cache secrets plus loopback host details. This is not a runtime feature change; it hardens the automation that will consume future live artifacts.

Commit `24306450` extends that acceptance boundary to relaxed Tailscale gates. A live run is now diagnostic, not production-ready, when direct-path checks, TUN ping checks, TCP checks, TCP ports, or full ping-count requirements are explicitly weakened. This closes a concrete robustness hole: a DERP-only or no-TCP-check run can still be useful for diagnosis, but it cannot satisfy the requested Tailscale production path.

Commit `3404b01c` extends the same acceptance boundary to relaxed OpenCode and cache-evidence gates. A live run is now diagnostic, not production-ready, when root OpenCode auth, stable credential key source, affinity stats, active-ready account count, `glm-5.2` model selection, cache stats, warm-up count, measured request count, request hit-rate threshold, stats hit-rate threshold, or cache-signal-token threshold are weakened below the production defaults. This matters because a permissive cache smoke can be useful while debugging a deployment, but it must not be consumed as evidence that the OpenCode subscription path is ready for maximum `glm-5.2` cache-hit validation.

Commit `0cdfdc74` brings the non-credentialed browser authorization path into the same production live E2E contract. The live gate now runs `opencode-auth-session-smoke` after a successful Tailscale gate and before root-only OpenCode preflight or cache smoke. Production acceptance requires the official OpenCode authorization entrypoint, screenshot capture, and sidecar lifecycle checks to pass. `--skip-auth-smoke true`, an `about:blank` smoke URL, or disabled screenshot checks are diagnostic-only and cannot produce production acceptance. This closes a proof gap between artifact rollout and final live E2E: a deployed service can no longer claim final readiness for the OpenCode path while the remote browser authorization bridge is unverified.

Commit `cdb13e4e` addresses the first real operator feedback on that bridge: the remote page was visible in Admin Web, but clicking the embedded browser image was not reliably actionable. The frontend now exposes the screenshot as a stable button-backed pointer surface instead of relying on the image node itself, while the coordinate mapper still enforces `object-contain` letterbox bounds. Successful interactions now schedule staged screenshot refreshes at 350 ms, 1250 ms, 2500 ms, and 5000 ms, so slow OAuth or Google navigation is not mistaken for a dead click. The sidecar click path now validates finite in-viewport points and sends `mouseMoved -> mousePressed -> mouseReleased` with button state. Remote post-apply smoke proved the installed sidecar can click a non-secret browser page and mutate page state; this is click-operability evidence, not real Google login evidence.

The follow-up stopped-session investigation found a different failure mode: the Admin Web can keep displaying an old screenshot after the remote browser session has stopped, making a stale bitmap look like an interactive browser. Recent server logs showed `/login/click` requests reaching the backend, while the sidecar status for that account was already `stopped` when checked through the configured state directory. The frontend now treats `status=stopped` / `running=false` as a screenshot invalidation signal: scheduled refreshes are cancelled, the stale screenshot is cleared, and explicit Stop also clears the viewport immediately. This does not make a dead Chromium session interactive; it removes the misleading stale surface and forces the operator to start/capture a fresh live browser before clicking.

The next click investigation narrowed the remaining failure to the screenshot coordinate contract rather than CDP click delivery. Server logs showed repeated successful `/login/click` requests, and a direct CDP click at the DOM center of the `Continue with Google` link moved the remote page into the Google OAuth target. The broken boundary was that the Admin Web mapped pointer positions against a fixed `1280x900` viewport while the actual remote page/screenshot observed during the live session was a different size. The sidecar now returns PNG-derived screenshot dimensions, the Go API preserves those dimensions, and the frontend refuses to map clicks unless the current screenshot carries valid width/height. This keeps `object-contain` letterbox math tied to the real remote bitmap instead of a deployment assumption.

Operator feedback after the embedded-panel fixes shows that the small in-page control surface still creates too much friction for Google login. The new direction is a same-origin popup route, `/opencode-browser`, backed by the same remote Chromium session and root-only OpenCode login APIs. This is intentionally not a local-browser login: local browser cookies would not be readable by the remote New API extractor. The popup gives the remote browser its own full-window control surface, account selector, status, screenshot, click, text input, safe key controls, extract/quota/activate/stop actions, and no application sidebar.

| Prior requirement | Current code state on `main` | Evidence paths | Remaining risk / next action |
|---|---|---|---|
| Keep the solution native to New API, not opencode-go/opencode-cc as runtime dependencies | Satisfied. New API now owns account records, root-only APIs, activation, frontend controls, and relay/cache measurement scripts. | `model/opencode_account.go`, `controller/opencode_account.go`, `router/opencode-account-router.go`, `web/default/src/features/opencode-accounts`, `scripts/glm-cache-smoke.mjs` | opencode-go/opencode-cc remain comparison sources only. Any future import of their behavior should stay behind New API service boundaries instead of calling them as sidecar business logic. |
| Remote browser authorization from the deployed server | Satisfied for infrastructure and smoke-tested without credentials. The sidecar starts isolated Chromium/Xvfb sessions and exposes status, screenshot, click, stdin text input, safe control-key press, extract, stop, and purge actions. The new runtime smoke runner exercises start/status/press/screenshot/stop/purge against a non-credentialed page and emits only fixed categories. | `scripts/opencode-auth-session.mjs`, `scripts/opencode-auth-session-smoke.mjs`, `service/opencode_browser_session.go`, `controller/opencode_account.go`, `web/default/src/features/opencode-accounts/api.ts` | Real Google/OpenCode authorization still requires operator-controlled subscription credentials and must remain outside the repository. If CDP interaction is insufficient in real login, add noVNC/streaming as a transport fallback without changing account or activation contracts. |
| Avoid collecting Google passwords directly | Satisfied. The connector opens the official OpenCode authorization entrypoint and the browser input path is an operator-controlled remote session. Text input is sent through stdin, not argv; safe control keys are whitelisted separately. | `scripts/opencode-auth-session.mjs`, `service/opencode_browser_session.go`, `service/opencode_browser_session_test.go` | Browser profile data is still sensitive runtime state. Keep it in the runtime state directory, never in repository artifacts or rollout logs. |
| Store OpenCode account material safely | Satisfied at repository level. Account secrets are encrypted, public responses are masked, status URLs/errors are sanitized, and diagnostics expose only key-source/readiness categories. | `common/crypto.go`, `model/opencode_account.go`, `controller/opencode_account.go`, `service/opencode_browser_session.go` | Production must use a stable `CRYPTO_SECRET`. Session-secret fallback is acceptable for test sessions but not for durable imported subscription accounts. |
...
| Make rollout repeatable and secret-clean | Satisfied for the rollout mechanism, and the latest browser-click hardening has been verified on the deployed service. The clean rollout helper builds from a clean checkout, runs targeted Node/Go/frontend/build gates, redacts public output, installs only non-secret runtime scripts including the auth-session runtime smoke runner, and applies only with explicit `--apply true`. Current remote evidence for pushed commit `cdb13e4e` includes a verification-only gate exit, a full apply transcript through source/build/artifact/service/restart/HTTP/auth-smoke gates with `deployed_prefix=cdb13e4e`, and independent post-apply smoke showing service health plus installed sidecar click validation. | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs`, `scripts/opencode-auth-session-smoke.mjs`, `scripts/opencode-live-e2e.mjs`, `scripts/opencode-auth-session.mjs` | Rollout helper does not own SSH transport, sudo provisioning, real account import, secret configuration, or tailnet reachability. Those remain operator/deployment responsibilities. Runtime marker and sidecar click smoke prove the non-secret browser-bridge/click path is deployed; they do not prove real OpenCode login, quota extraction, or cache billing. |
...
| OpenCode E2E preflight runner | Implemented and remotely applied | Added `scripts/opencode-e2e-preflight.mjs` plus Node tests. The runner performs non-mutating GET checks for `/api/status`, root-only OpenCode diagnostics, OpenCode account readiness summary, and the channel-affinity usage stats endpoint required by the later cache smoke. It reads root auth only from environment variables, emits only endpoint status, credential key-source class, account counts, fixed credential-integrity category counts, missing-field category counts, safe affinity-stats identity, and the count of accounts that are both active and consistently activation-ready. It exits non-zero when required root auth, stable credential key, diagnostics payload contract, account-list array contract, account item contract, account readiness consistency, affinity stats readability, affinity stats identity, minimum activation-ready account counts, minimum active account counts, or minimum active-ready account counts are not satisfied. The account item contract requires fixed public field types before summary counting: boolean `active`, boolean `activation_ready`, string `credential_integrity`, string `credential_key_source`, and array `missing_activation_fields`. This prevents truthy strings or malformed missing-field payloads from being counted as deployable readiness evidence. The active-ready gate prevents a false pass where one account is active while a different account is activation-ready, and the contract gates prevent malformed diagnostics/accounts/stats payloads from being treated as deployable. The readiness gate also refuses account payloads that claim `activation_ready` while credential integrity is not `ok` or missing fields are still present. Credential-integrity values from account payloads are mapped into fixed categories before summary output, so a malformed backend string is not echoed. Endpoint errors redact the configured deployment URL plus generic secret-shaped fragments such as bearer tokens, OAuth `code`/`state`, API keys, cookies, workspace IDs, emails, and local absolute paths. Pushed `main` commit `7841fd58` deployed the account item contract gate. This turns the first post-rollout verification step into a machine gate without printing root tokens, cookies, raw emails, workspace IDs, raw missing-field names, account secrets, deployment URLs, or local deployment paths. |
...
| OpenCode live E2E orchestration gate | Implemented and remotely applied | Added `scripts/opencode-live-e2e.mjs` plus Node tests. The runner composes `tailscale-link-preflight`, `opencode-e2e-preflight`, and `glm-cache-smoke` with strict production-oriented defaults: at least one active-ready OpenCode account, stable credential key source, readable affinity stats, two warm-up calls, six measured calls, request and stats hit-rate thresholds of `0.8`, at least one cache-signal token, and a Tailscale TCP check for the deployed New API service port. Tailscale failure stops the run before credentialed OpenCode/cache gates by default, so a known-broken tailnet does not consume live API quota or produce misleading cache evidence. `--continue-on-failure true` and `--skip-tailscale true` are explicit diagnostic overrides, not production acceptance modes. The runner now emits a separate `acceptance` summary: `production_ready` is true only when all stage checks pass and no diagnostic override is active, and the CLI exits non-zero when acceptance fails even if diagnostic stage checks passed. Relaxed Tailscale requirements are now also diagnostic overrides: disabling direct-path, TUN, TCP, clearing TCP ports, or lowering `min-pongs` below `ping-count` prevents production acceptance even if stage checks pass. This prevents skipped-Tailscale, DERP-tolerant, or no-TCP-check diagnostic runs from being consumed as final acceptance. The live gate treats any actually executed OpenCode/cache stage that returns `skipped` or omits `checks.status` as failed; `skipped` is accepted only for explicitly disabled Tailscale or downstream stages blocked by an earlier failed gate. It also adds `opencode_activation_contract_ready` after the preflight stage and requires `accounts.activation_contract.ready >= minActiveReadyAccounts`; if that category evidence is missing or below threshold, `glm_cache_smoke` is skipped before live quota is spent. Stage exceptions are converted into failed stage summaries with redacted deployment URL, Tailscale target, admin/relay credentials, prompt cache key, input text, OAuth payloads, emails, and local paths, preserving fail-fast behavior without printing raw exception text. Successful stage summaries are also recursively sanitized at the orchestration boundary, including both JSON keys and values, so a buggy lower-level runner cannot reintroduce raw live-run material into the combined JSON output. The sanitizer now covers prompt echo variants as well: raw prefixes, full JSON strings, JSON inner escaped strings, and JSON inner prefixes. The live E2E default intentionally does not require a remote Deskflow server port, because the current Deskflow topology is remote-client to local-server/proxy. The clean rollout helper now includes this script's tests and syntax check in its Node gate. Pushed `main` commit `713392b0` deployed the production-acceptance summary; follow-up commit `300ea33d` adds CLI-level coverage proving diagnostic overrides make the CLI exit non-zero even when diagnostic stage checks pass; commit `24306450` deploys relaxed-Tailscale override detection. |
| OpenCode/cache evidence acceptance boundary | Implemented and remotely applied | Commit `3404b01c` adds regression coverage and runtime logic that mark relaxed OpenCode/cache evidence as diagnostic. The fixed override names cover optional root auth, optional stable credential key source, optional affinity stats, relaxed active-ready account count, non-`glm-5.2` cache model, disabled/optional stats, reduced warm-up or measured request count, reduced request/stats hit-rate thresholds, and reduced cache-signal-token threshold. Local Node gate now passes with 97 tests across cache smoke, preflight, auth sidecar, rollout, Tailscale preflight, and live E2E orchestration. Remote verification-only and full apply both passed from a clean checkout, and post-apply smoke confirmed the installed live E2E script carries the relaxed evidence marker. | `scripts/opencode-live-e2e.mjs`, `scripts/opencode-live-e2e.test.mjs`, `scripts/new-api-clean-rollout.mjs` | This proves weak evidence cannot be mislabeled as production acceptance. It does not prove real OpenCode login, real quota extraction, upstream cache billing, or Tailscale direct-path quality. |
| Official auth-session live E2E gate | Implemented, pushed, and remotely verified | Commit `0cdfdc74` adds `opencode_auth_smoke` as a first-class live E2E stage. The stage runs after Tailscale and before root-only OpenCode/cache gates, converts auth-session smoke booleans into normal `checks.status` items, fails fast before credentialed gates when the browser bridge is broken, and treats skipped auth smoke, non-official auth URLs, or disabled screenshots as diagnostic overrides. The same commit expands redaction so exact secret object keys such as `token` and `prompt_cache_key` have their values replaced, while token accounting fields such as `cached_tokens` remain usable evidence. Local Node rollout gate passes with 99 tests. Remote post-apply smoke confirms the deployed runtime carries the auth gate marker, the auth-session smoke script passes syntax checks, the redaction marker is present, and an official OpenCode authorization-page smoke succeeds without credentials. | `scripts/opencode-live-e2e.mjs`, `scripts/opencode-live-e2e.test.mjs`, `scripts/opencode-auth-session-smoke.mjs`, `scripts/opencode-auth-session-smoke.test.mjs` | This proves the production acceptance contract now covers the deployed browser bridge. It still does not replace a real operator login, account extraction, channel activation, or `glm-5.2` cache-hit E2E. |
...
| Remote browser page-title status metadata | Implemented and remotely applied | Login status now carries a sanitized `title` field from the remote browser target. The Node sidecar strips URLs, bearer-shaped fragments, secret key/value fragments, emails, and local paths before emitting status JSON, while the Go service applies the same public-output boundary again before API responses. The Admin UI prefers title over URL for the compact status label, which helps the operator distinguish OpenCode, Google, callback, and blank pages without exposing authorization payloads. Code commit `e6f4b229` passed Node, Go, frontend helper, cross-package Go, typecheck, targeted oxlint, and default frontend build coverage; pushed `main` commit `d6f31cbe` then passed remote clean rollout verification-only and full apply. |
| Auth sidecar runtime smoke gate | Implemented and remotely applied | Added `scripts/opencode-auth-session-smoke.mjs`, a non-credentialed runtime smoke that starts the sidecar against `about:blank`, verifies running status, sends a whitelisted Escape key, verifies PNG screenshot output, stops the session, and purges browser artifacts. The report exposes only fixed status categories and booleans; it does not print browser URL/title, screenshot bytes, state paths, account material, or deployment endpoints. The clean rollout helper installs this runner and includes its tests plus syntax check in the Node gate. Code commit `7aa5d64f` passed the full local Node rollout gate with 89 subtests, and commit `3e030e09` fixes the runner's symlinked artifact entrypoint after the first post-apply check showed the installed script could exit without running `main()`. Pushed `main` commit `0dbd7964` passed remote verification-only and full apply; independent post-apply checks confirmed the installed runner emits a successful JSON summary with start/status/press/screenshot/stop checks all true. Commit `c60a4dc0` makes this smoke a default clean-rollout apply gate rather than a manual post-apply step, and pushed `main` commit `243055ca` verified the full apply path now emits `auth_runtime_smoke=ok` before the deployed prefix. |
| Xvfb display collision retry | Implemented and remotely applied | The sidecar no longer treats the account-derived Xvfb display as a single point of failure. On Linux hosts without an existing `DISPLAY`, `login/start` now probes a bounded sequence of account-derived display candidates and retries only the retryable `Xvfb exited before ready` startup failure, while still cleaning the browser/Xvfb processes spawned by each failed attempt. This addresses a real remote runtime failure caused by a display collision without hiding missing-browser or missing-Xvfb dependency failures. Regression coverage locks the candidate sequence, the temporary remote candidate smoke passed with the new retry path, and the pushed `main` commit `0dbd7964` passed clean rollout plus installed auth runtime smoke after deployment. |
...
| Codex OAuth credential extraction | Implemented and remotely applied | The extractor now recognizes complete Codex OAuth credential objects from storage or same-site JSON responses. It accepts `access_token` only when paired with `account_id`, or when the account ID can be derived from the access-token JWT claim, then serializes the known fields into the existing Codex channel JSON key contract. Standalone OAuth token fields remain rejected as API-key candidates. Service tests cover extraction, JWT-derived account identity, and activation of a bound Codex channel with the extracted credential. Pushed `main` commit `98b804ee` passed verification-only gates, full apply, and post-apply service/runtime-script smoke on the remote service. |
...
| Frontend account window | Implemented | Added Root-only admin route, sidebar entry, account list, enabled-channel selector with numeric ID fallback, remote screenshot controls, safe control-key buttons, extract, quota refresh, activate, stop, and delete actions. The account list now has a fixed create/bind toolbar plus an independently scrollable table region, so importing many OpenCode accounts does not push the remote browser workspace out of the fixed admin surface. The embedded browser screenshot is now hosted inside a stable button-backed click surface so operator clicks target the interaction area consistently. |
...
| Frontend screenshot viewport contract | Implemented locally | The sidecar now parses PNG width/height from `Page.captureScreenshot` output and returns them with `image_base64`; the Go screenshot contract preserves those dimensions; the Admin Web stores the screenshot as `{ imageBase64, width, height }` and maps pointer coordinates against the current bitmap dimensions. Missing or invalid dimensions clear the screenshot instead of falling back to a stale hard-coded viewport. This fixes the proven coordinate-mapping root cause, but still needs remote rollout and an operator-driven real Google login attempt before claiming real account authorization is complete. If Google later blocks embedded CDP-style interaction for policy or anti-abuse reasons, the better fallback is a standalone remote-browser viewer backed by the same remote Chromium profile, not local-browser cookie extraction. |
| Remote browser popup window | Implemented locally | The Admin Web can open `/opencode-browser?account_id=<id>` in a same-origin popup sized to the available screen. The popup is guarded as a super-admin route but avoids the normal authenticated sidebar layout, so Google/OpenCode interaction gets a dedicated viewport. It reuses the same remote session APIs and screenshot-dimension mapper, supports account switching inside the popup, and keeps URL state limited to the non-secret numeric account ID. | This is the preferred near-term fallback for difficult Google interaction. It still needs remote rollout and a real operator login attempt. A future noVNC/streaming transport should only be added if this full-window CDP viewer still cannot satisfy Google interaction requirements. |
...
The extractor now distinguishes incomplete OAuth artifacts from complete Codex credentials. A bare `access_token` is still not enough to become an API key; however, an object that satisfies the Codex channel contract (`access_token` plus `account_id`) is serialized into the same JSON key shape already consumed by the Codex relay. When the object omits `account_id`, the extractor may derive it from the access-token JWT claim using the existing Codex OAuth parser boundary. This narrows a real live-login gap: if the OpenCode page exposes a Codex-compatible credential object instead of an `api_key` field, the account can become activation-ready without weakening the OAuth-token false-positive guard.
...
The latest live E2E gate consumes those categories instead of re-implementing readiness. This is the right ownership split: New API service/controller code decides whether an account can be activated, preflight summarizes that public state into fixed categories, and live E2E refuses to run cache smoke unless the same summary proves at least one `ready` activation contract. The tradeoff is intentionally strict. A deployment that reports `active_ready` but omits `activation_contract.ready` now fails before spending real `glm-5.2` quota; operators must repair the account/channel/Codex credential contract first or run an explicit diagnostic override outside production acceptance.

The newest preflight hardening moves another assumption to the API boundary: account item fields must have the exact types required by readiness accounting. This is not extra defensive checking inside trusted code; it validates the external HTTP payload before it becomes an internal summary. The important failure mode is JavaScript truthiness. Without this gate, a string `active` or non-array `missing_activation_fields` could distort account counts before the later active-ready and activation-contract checks run. The new `opencode_accounts_item_contract` check fails that payload explicitly and still reports only fixed field-category counts.

The newest live E2E hardening makes acceptance explicit instead of asking operators or automation to infer it from stage checks. `checks.status` still describes whether executed stages passed, while `acceptance.production_ready` describes whether the run is admissible as the production gate. This matters because `--skip-tailscale true` can be useful for diagnosing account/cache behavior but contradicts the requirement that the robust path must go through Tailscale; the same rule now applies to `--skip-auth-smoke true` and non-official auth smoke URLs because they do not prove the deployed browser bridge can reach the official OpenCode authorization path. The tradeoff is stricter CLI behavior: a diagnostic run may now exit non-zero even when the diagnostic stages themselves passed. That is intentional; diagnostic output is still useful JSON, but it is not a deployable acceptance signal.
...
```text
RED: bun test src/features/opencode-accounts/lib.test.ts
  failed because shouldClearOpenCodeLoginScreenshotForStatus was missing
...
post-apply runtime probe:
  rollout_process=absent
  service_active=ok
  http_smoke=ok
  live_e2e_node_check=ok
  auth_smoke_node_check=ok
  live_e2e_auth_gate=ok
  auth_smoke_redaction=ok
  auth_runtime_smoke=ok
  cleanup_ok
```

The apply command was interrupted before returning a complete helper transcript, so this document does not claim a full `deployed_prefix` line for this revision. The acceptance evidence is the deployed runtime state: service health, installed script syntax, auth gate marker, redaction marker, official OpenCode auth-session smoke success, and cleanup.

Latest remote rollout verification for pushed commit `24306450`:
...
clean rollout apply:
  ...
  http_smoke=ok
  auth_runtime_smoke=ok
  deployed_prefix=24306450

post-apply smoke:
  service_active=ok
  http_smoke=ok
  live_e2e_node_check=ok
  relaxed_tailscale_acceptance_marker=ok
  cleanup_ok
```

Latest remote verification-only for pushed commit `300ea33d`:
...
Latest remote rollout for pushed commit `3404b01c`:
...
clean rollout apply:
  ...
  http_smoke=ok
  auth_runtime_smoke=ok
  deployed_prefix=3404b01c

post-apply smoke:
  service_active=ok
  http_smoke=ok
  live_e2e_node_check=ok
  relaxed_evidence_acceptance_marker=ok
  cleanup_ok
```

Previous runtime rollout verification for pushed commit `713392b0`:
...
clean rollout apply:
  ...
  http_smoke=ok
  auth_runtime_smoke=ok
  deployed_prefix=713392b0

post-apply smoke:
  service_active=ok
  http_smoke=ok
  runtime scripts node --check=ok
  live_e2e_acceptance_marker=ok
  cleanup_ok
```
...
Remote clean artifact rollout:
  ...
  independent post-apply smoke confirmed service active, local HTTP status OK, installed runtime script syntax OK, and auth sidecar empty-state status OK
  clean rollout helper verification-only gate for pushed commit 16c126d1 passed git_clone/revision/node_scripts/go_targeted including the cross-package Go gate/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper full remote apply for pushed commit 16c126d1 passed git_clone/revision/node_scripts/go_targeted including the cross-package Go gate/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke
  independent post-apply smoke confirmed service active, local HTTP status OK, installed runtime script syntax OK, and auth sidecar empty-state status OK
  clean rollout helper verification-only gate for pushed commit 8c5d4e25 passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper full remote apply for pushed commit 8c5d4e25 passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke/auth_runtime_smoke
  independent post-apply smoke confirmed service active, local HTTP status OK, and installed runtime script syntax OK for auth sidecar, OpenCode preflight, cache smoke, Tailscale preflight, live E2E orchestrator, and clean rollout helper
  temporary rollout helper used for the 8c5d4e25 deployment was removed after successful verification
  clean rollout helper verification-only gate for pushed commit 98b804ee passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper full remote apply for pushed commit 98b804ee passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke/auth_runtime_smoke
  independent post-apply smoke confirmed service active, local HTTP status OK, and installed runtime script syntax OK for auth sidecar, OpenCode preflight, cache smoke, Tailscale preflight, live E2E orchestrator, and clean rollout helper
  temporary rollout helper used for the 98b804ee deployment was removed after successful verification
  clean rollout helper verification-only gate for pushed commit 300ea33d passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact, apply was skipped, and the temporary helper was cleaned up
  clean rollout helper verification-only gate for pushed commit 24306450 passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper full remote apply for pushed commit 24306450 passed git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke/auth_runtime_smoke
  independent post-apply smoke confirmed service active, local HTTP status OK, installed live E2E script syntax OK, relaxed Tailscale acceptance marker present, and temporary helper cleanup OK
```
...
- The stopped-session screenshot lifecycle fix proves the Admin Web no longer keeps presenting a stale bitmap as an interactive browser after status reports stopped. It does not prove why a browser session stopped, and it does not replace browser lifetime diagnostics if Chromium exits during a real Google login.
...
- The CLI diagnostic-exit regression for commit `300ea33d` uses a fake local New API server. It proves process exit semantics, summary shape, cache-stage composition, and redaction at the CLI boundary; it is not live-account, live-quota, or upstream cache-billing evidence.
- The relaxed-Tailscale acceptance regression for commit `24306450` proves that weakened Tailscale gates cannot be mistaken for production acceptance. It does not prove that the current tailnet has direct path; it only prevents diagnostic relaxations from being promoted to final evidence.
- The relaxed OpenCode/cache evidence regression for commit `3404b01c` proves that weakened account-readiness, credential-key, affinity-stats, model, stats, request-count, hit-rate, or cache-signal gates cannot be mistaken for production acceptance. It does not prove a real OpenCode subscription account is imported, activated, or getting upstream warm-cache billing.
- The official auth-session live E2E regression for commit `0cdfdc74` proves that production acceptance now requires the non-credentialed official OpenCode browser bridge stage before root-only OpenCode/cache gates. The deployed runtime now carries that gate and has passed a non-credentialed official OpenCode auth-session smoke. This still does not prove real login, account extraction, channel activation, or upstream cache billing.
...
2. Before any account import or live cache test, run the deployed-service preflight against the current runtime and keep the clean rollout helper as the repeatable resync path for future pushed revisions. Use verification-only first; only use `--apply true` when intentionally replacing the running artifact:

```bash
node scripts/new-api-clean-rollout.mjs \
  --revision <pushed-main-revision> \
  --apply false
```
...
本次更新把 live E2E artifact 中的生产验收与诊断执行明确分离。`scripts/opencode-live-e2e.mjs` 现在会输出 `acceptance` 摘要，包含 `production_ready`、`mode`、`diagnostic_overrides` 和 `failed_checks`。只有所有 stage checks 通过且没有诊断 override 时，运行结果才能被视为 production-ready；`--skip-tailscale true` 或 `--continue-on-failure true` 即使让个别诊断阶段完成，也会得到 `acceptance.status=failed`。这可以避免本地或局部诊断结果被误当成完整 Tailscale/OpenCode/cache 验收，同时不打印账号 label、channel 名、masked email、原始 missing-field 名、API key、cookie、OAuth token、workspace ID、部署地址或原始网络标识。

后续提交 `300ea33d` 为同一边界补上 CLI 级回归覆盖。测试用契约完整的 fake New API server 驱动 `opencode-live-e2e.mjs --skip-tailscale true`，验证 stage checks 可以通过但 `acceptance` 仍保持 diagnostic/failed，确认 CLI 退出码为 `1`，检查 cache-smoke stage 仍能记录测量命中，并断言 stdout 不包含 relay/admin/cache secret 或 loopback host 细节。这不是运行时功能变更；它加固的是后续消费真实 live artifact 的自动化边界。

提交 `24306450` 继续把这个 acceptance 边界扩展到被放宽的 Tailscale gate。现在只要 direct-path 检查、TUN ping 检查、TCP 检查、TCP 端口列表或完整 ping-count 要求被显式削弱，本次 live run 就只能是 diagnostic，不能 production-ready。这样补上了一个具体稳健性漏洞：DERP-only 或无 TCP 检查的运行结果仍可用于诊断，但不能满足用户要求的 Tailscale 生产路径。

提交 `3404b01c` 把同一条 acceptance 边界继续扩展到被放宽的 OpenCode 与 cache 证据 gate。只要 root OpenCode auth、稳定 credential key source、affinity stats、active-ready 账号数、`glm-5.2` 模型选择、cache stats、warm-up 次数、测量请求次数、request hit-rate 阈值、stats hit-rate 阈值或 cache-signal-token 阈值低于生产默认值，本次 live run 就只能是 diagnostic，不能 production-ready。这一点很关键：宽松 cache smoke 可以用于部署诊断，但不能被消费成 OpenCode 套餐链路已经具备最大化 `glm-5.2` 缓存命中验证条件的证据。

提交 `0cdfdc74` 把无凭据浏览器授权链路纳入同一个生产 live E2E 契约。live gate 现在会在 Tailscale gate 通过后、root-only OpenCode preflight 与 cache smoke 之前运行 `opencode-auth-session-smoke`。生产验收要求官方 OpenCode 授权入口、截图获取和 sidecar 生命周期检查都通过。`--skip-auth-smoke true`、`about:blank` smoke URL 或关闭截图检查只能作为 diagnostic，不能产生 production acceptance。这样补上了 artifact rollout 与最终 live E2E 之间的证明缺口：远端浏览器授权桥未验证时，部署服务不能声称 OpenCode 链路已经达到最终验收状态。

提交 `cdb13e4e` 处理了该浏览器桥收到的第一类真实操作者反馈：Admin Web 中能看到远端页面，但嵌入截图并不可靠可点。前端现在把截图放在稳定的 button-backed pointer surface 中，而不是依赖 img 节点自身；坐标映射仍然保留 `object-contain` letterbox 边界。成功交互后会在 350 ms、1250 ms、2500 ms 和 5000 ms 分阶段刷新截图，避免 OAuth 或 Google 跳转较慢时被误判为点击无效。sidecar click 路径现在会校验有限且位于 viewport 内的坐标，并按 `mouseMoved -> mousePressed -> mouseReleased` 带 button state 发送 CDP 鼠标事件。远端 apply 后 smoke 已证明已安装 sidecar 可以点击无凭据页面并改变页面状态；这只是点击可操作性证据，不是真实 Google 登录证据。

后续 stopped-session 调查发现了另一类故障模式：远端浏览器会话停止后，Admin Web 仍可能显示旧截图，让一张 stale bitmap 看起来像仍可交互的浏览器。最近服务日志证明 `/login/click` 请求已经进入后端，而按服务实际配置的 state 目录读取同一账号 sidecar 状态时已经是 `stopped`。前端现在把 `status=stopped` / `running=false` 视为截图失效信号：取消已调度的延迟刷新、清空旧截图，显式 Stop 成功后也立即清空 viewport。该修复不会让已经死亡的 Chromium 会话恢复交互；它移除误导性的旧截图表面，强制操作者重新 start/capture 一个真实运行中的浏览器再点击。

后续点击调查把剩余故障收敛到截图坐标契约，而不是 CDP 点击投递本身。服务日志显示多次 `/login/click` 请求成功进入后端；直接用 CDP 在 `Continue with Google` 链接的 DOM 中心点点击，可以把远端页面推进到 Google OAuth target。真正破损的边界是 Admin Web 按固定 `1280x900` viewport 换算鼠标位置，而真实会话中观察到的远端页面/截图尺寸并不等于该固定值。sidecar 现在从 PNG 截图头部返回真实宽高，Go API 保留这些尺寸，前端只有在当前截图带合法 width/height 时才允许映射点击。这样 `object-contain` letterbox 计算绑定到真实远端 bitmap，而不是部署假设。

嵌入式面板修复后的操作者反馈说明，小尺寸页面内控制面仍然显著影响 Google 登录操作。新的方向是同源弹出窗口 route：`/opencode-browser`，仍然复用同一个远端 Chromium 会话和 root-only OpenCode 登录 API。这不是本地浏览器登录：本地浏览器 cookie/profile 不能被远端 New API extractor 自动读取。popup 给远端浏览器单独的全窗口控制面，包含账号选择、状态、截图、点击、文本输入、安全控制键、extract/quota/activate/stop 动作，并且不带应用侧边栏。

| 先前要求 | 当前 `main` 代码状态 | 证据路径 | 剩余风险 / 下一步 |
|---|---|---|---|
| 方案必须内建在 New API 中，而不是运行时依赖 opencode-go/opencode-cc | 已满足。New API 现在拥有账号记录、root-only API、激活流程、前端控制面和 relay/cache 测量脚本。 | `model/opencode_account.go`, `controller/opencode_account.go`, `router/opencode-account-router.go`, `web/default/src/features/opencode-accounts`, `scripts/glm-cache-smoke.mjs` | opencode-go/opencode-cc 只应继续作为对照来源。后续吸收它们的行为时，应落在 New API service 边界内，而不是把它们变成业务 sidecar。 |
| 在部署服务器上执行远端浏览器授权 | 基础设施已满足，并已完成无凭证 smoke。sidecar 可启动隔离 Chromium/Xvfb 会话，并提供 status、screenshot、click、stdin 文本输入、安全控制键 press、extract、stop、purge。新的 runtime smoke runner 会对无凭证页面执行 start/status/press/screenshot/stop/purge，并且只输出固定类别。 | `scripts/opencode-auth-session.mjs`, `scripts/opencode-auth-session-smoke.mjs`, `service/opencode_browser_session.go`, `controller/opencode_account.go`, `web/default/src/features/opencode-accounts/api.ts` | 真实 Google/OpenCode 授权仍需要操作者控制的订阅账号，不能进入仓库。如果真实登录证明 CDP 操作不足，再增加 noVNC/streaming 作为传输兜底，不改变账号模型和激活契约。 |
| 不直接收集 Google 密码 | 已满足。连接器打开官方 OpenCode 授权入口，浏览器输入是操作者控制的远端会话。文本通过 stdin 传给 sidecar，不进 argv；安全控制键另走白名单。 | `scripts/opencode-auth-session.mjs`, `service/opencode_browser_session.go`, `service/opencode_browser_session_test.go` | 浏览器 profile 运行时状态仍然敏感。它必须留在运行时状态目录中，不能进入仓库 artifact 或 rollout 日志。 |
| 安全存储 OpenCode 账号材料 | 仓库层面已满足。账号 secret 加密存储，公开响应只给 masked view，状态 URL/错误信息会脱敏，diagnostics 只暴露 key-source/readiness 类别。 | `common/crypto.go`, `model/opencode_account.go`, `controller/opencode_account.go`, `service/opencode_browser_session.go` | 生产必须配置稳定 `CRYPTO_SECRET`。session-secret fallback 只能用于测试会话，不能用于持久导入的订阅账号。 |
...
| 最大化 `glm-5.2` 套餐缓存命中概率与统计质量 | 仓库前置条件已实现。Codex affinity 覆盖 `glm-*`，同步 `Session_id` 与 `prompt_cache_key`，对原始 affinity 值做 hash，保留 cached-token usage 字段，提供带 stats identity gate 的脱敏多轮 smoke runner，并且现在会在 cache smoke 前要求 live preflight 的 `activation_contract.ready` 计数达标。 | `service/relayconvert`, `model/channel_affinity.go`, `scripts/glm-cache-smoke.mjs`, `scripts/opencode-live-e2e.mjs` | 这仍不等于证明上游 warm-cache 计费。证明必须使用真实导入的 OpenCode 订阅账号、已激活 channel、多轮 `glm-5.2` 请求，以及 live E2E gate 返回的 `checks.status`。 |
...
| rollout 可重复且不携带 secret | rollout 机制层面已满足，且最新浏览器点击加固已在部署态服务上验证。clean rollout helper 从干净 checkout 构建，执行定向 Node/Go/前端/build gate，脱敏公开输出，只安装非 secret runtime scripts，包括 auth-session runtime smoke runner，且只有显式 `--apply true` 才切换运行 artifact。当前已推送提交 `cdb13e4e` 的远端证据包括 verification-only gate 退出成功、完整 apply transcript 通过 source/build/artifact/service/restart/HTTP/auth-smoke gate 并报告 `deployed_prefix=cdb13e4e`，以及 apply 后独立 smoke 证明服务健康和已安装 sidecar click validation。 | `scripts/new-api-clean-rollout.mjs`, `scripts/new-api-clean-rollout.test.mjs`, `scripts/opencode-auth-session-smoke.mjs`, `scripts/opencode-live-e2e.mjs`, `scripts/opencode-auth-session.mjs` | rollout helper 不负责 SSH 传输、sudo provisioning、真实账号导入、secret 配置或 tailnet 可达性；这些仍是操作者/部署责任。runtime marker 与 sidecar click smoke 证明无 secret 的浏览器桥/click 路径已部署，不证明真实 OpenCode 登录、quota 提取或 cache 计费。 |
...
| OpenCode E2E preflight runner | 已实现并完成远端 apply | 已新增 `scripts/opencode-e2e-preflight.mjs` 及 Node 测试。runner 会对 `/api/status`、root-only OpenCode diagnostics、OpenCode 账号 readiness 汇总，以及后续 cache smoke 依赖的 channel-affinity usage stats endpoint 执行非破坏性 GET 检查。它只从环境变量读取 root auth，输出只包含 endpoint 状态、credential key-source 类别、账号计数、固定 credential-integrity 类别计数、missing-field 类别计数、安全的 affinity-stats identity，以及同时处于 active 与一致 activation-ready 状态的账号数；当 root auth、稳定 credential key、diagnostics payload 契约、账号列表数组契约、账号 item 契约、账号 readiness 一致性、affinity stats 可读性、affinity stats identity、最小 activation-ready 账号数、最小 active 账号数或最小 active-ready 账号数不满足要求时非零退出。账号 item 契约会在进入摘要计数前要求固定公开字段类型：布尔 `active`、布尔 `activation_ready`、字符串 `credential_integrity`、字符串 `credential_key_source` 和数组 `missing_activation_fields`。这能避免 truthy 字符串或畸形 missing-field payload 被误算成可部署 readiness 证据。active-ready gate 可避免一个账号 active、另一个账号 activation-ready 时误判为可执行真实 cache smoke；payload/identity gate 可避免畸形 diagnostics/accounts/stats 响应被误当成可上线状态。readiness gate 还会拒绝声称 `activation_ready`、但 credential integrity 不是 `ok` 或仍存在 missing fields 的账号 payload。账号 payload 中的 credential-integrity 值会先映射为固定类别再进入摘要，因此畸形后端字符串不会被原样回显。endpoint 错误会脱敏已配置部署 URL，以及 bearer token、OAuth `code`/`state`、API key、cookie、workspace ID、邮箱和本地绝对路径等通用敏感形态。已推送的 `main` 提交 `7841fd58` 已部署 account item contract gate。这样远端 rollout 后的第一步验证可以变成机器门，同时不打印 root token、cookie、原始邮箱、workspace ID、原始 missing-field 名称、账号 secret、部署 URL 或本地部署路径。 |
...
| OpenCode live E2E 编排 gate | 已实现并完成远端 apply | 已新增 `scripts/opencode-live-e2e.mjs` 及 Node 测试。runner 会用面向生产的严格默认值串联 `tailscale-link-preflight`、`opencode-e2e-preflight` 与 `glm-cache-smoke`：至少一个 active-ready OpenCode 账号、稳定 credential key source、可读 affinity stats、2 次 warm-up、6 次测量请求、request 与 stats 命中率阈值 `0.8`、至少一个 cache-signal token，以及部署态 New API 服务端口的 Tailscale TCP 检查。默认情况下 Tailscale 失败会在进入带真实凭据的 OpenCode/cache gate 前停止，因此已知坏的 tailnet 不会继续消耗真实 API quota，也不会生成误导性的 cache 证据。`--continue-on-failure true` 与 `--skip-tailscale true` 是显式诊断开关，不是生产验收模式。runner 现在会额外输出独立的 `acceptance` 摘要：只有所有 stage checks 通过且没有诊断 override 时，`production_ready` 才为 true；只要 acceptance 失败，CLI 就会非零退出，即使诊断 stage checks 本身通过。被放宽的 Tailscale 要求现在同样属于诊断 override：关闭 direct-path、TUN、TCP，清空 TCP ports，或把 `min-pongs` 降到 `ping-count` 以下，即使 stage checks 通过，也不能形成 production acceptance。这样 skipped-Tailscale、容忍 DERP 或无 TCP 检查的诊断运行不会被消费成最终验收。live gate 会把实际执行的 OpenCode/cache stage 返回 `skipped` 或缺失 `checks.status` 视为失败；只有显式禁用 Tailscale 或被前置失败阻断的 downstream stage 才允许保留 `skipped`。它还会在 preflight stage 后增加 `opencode_activation_contract_ready`，要求 `accounts.activation_contract.ready >= minActiveReadyAccounts`；如果该类别证据缺失或低于阈值，`glm_cache_smoke` 会在消耗 live quota 前被跳过。stage 异常现在会被转换成 failed stage summary，并在进入摘要前脱敏部署 URL、Tailscale target、admin/relay 凭据、prompt cache key、input 文本、OAuth payload、邮箱和本地路径；这样保留 fail-fast 行为，但不打印原始异常文本。成功 stage summary 也会在编排层递归脱敏，包括 JSON key 和 value，因此即使底层 runner 有 bug，也不能把 live-run 原始材料重新带回组合 JSON 输出。sanitizer 现在还覆盖 prompt 回显变体：raw 前缀、完整 JSON 字符串、JSON 内层转义字符串和 JSON 内层前缀。live E2E 默认不再要求远端 Deskflow server 端口，因为当前 Deskflow 拓扑是 remote-client 到 local-server/proxy。clean rollout helper 现在会在 Node gate 中覆盖该脚本的测试与语法检查。已推送的 `main` 提交 `713392b0` 已部署 production-acceptance summary；后续提交 `300ea33d` 增加 CLI 级覆盖，证明诊断 override 即使让 stage checks 通过，也会让 CLI 非零退出；提交 `24306450` 已部署 relaxed-Tailscale override 检测。 |
| OpenCode/cache 证据 acceptance 边界 | 已实现并完成远端 apply | 提交 `3404b01c` 增加回归覆盖与运行时逻辑，把被放宽的 OpenCode/cache 证据标记为 diagnostic。固定 override 类别覆盖 optional root auth、optional stable credential key source、optional affinity stats、放宽 active-ready 账号数、非 `glm-5.2` cache model、stats disabled/optional、降低 warm-up 或测量请求数、降低 request/stats hit-rate 阈值，以及降低 cache-signal-token 阈值。本地 Node gate 现在在 cache smoke、preflight、auth sidecar、rollout、Tailscale preflight 与 live E2E orchestration 上共 97 个测试通过。远端 clean checkout 的 verification-only 与完整 apply 均通过，apply 后独立 smoke 确认已安装 live E2E 脚本包含 relaxed evidence marker。 | `scripts/opencode-live-e2e.mjs`, `scripts/opencode-live-e2e.test.mjs`, `scripts/new-api-clean-rollout.mjs` | 这证明弱证据不会被误标为生产验收；它不证明真实 OpenCode 登录、真实 quota 提取、上游 cache 计费或 Tailscale direct path 质量。 |
| 官方 auth-session live E2E gate | 已实现、已推送并完成远端验证 | 提交 `0cdfdc74` 把 `opencode_auth_smoke` 增加为一等 live E2E stage。该 stage 在 Tailscale 之后、root-only OpenCode/cache gate 之前运行，把 auth-session smoke 的布尔检查转换成标准 `checks.status` item；如果浏览器桥失败，会在进入带真实凭据的 gate 前 fail-fast；跳过 auth smoke、使用非官方 auth URL 或关闭截图检查都会进入 diagnostic override。同一提交扩展了脱敏边界：`token`、`prompt_cache_key` 等精确 secret 对象键的值会被替换，同时 `cached_tokens` 这类计量字段仍可作为证据保留。本地 Node rollout gate 共 99 个测试通过。远端 post-apply smoke 确认部署态 runtime 携带 auth gate marker，auth-session smoke 脚本通过语法检查，脱敏 marker 存在，并且官方 OpenCode 授权页无凭据 smoke 成功。 | `scripts/opencode-live-e2e.mjs`, `scripts/opencode-live-e2e.test.mjs`, `scripts/opencode-auth-session-smoke.mjs`, `scripts/opencode-auth-session-smoke.test.mjs` | 这证明部署态浏览器桥已纳入生产 acceptance 契约；仍不能替代真实操作者登录、账号提取、channel 激活或 `glm-5.2` cache-hit E2E。 |
...
| 远端浏览器页面标题状态元数据 | 已实现并完成远端 apply | login status 现在会携带来自远端浏览器 target 的脱敏 `title` 字段。Node sidecar 在输出 status JSON 前会移除 URL、bearer-like 片段、secret key/value 片段、邮箱和本地路径，Go service 在 API 响应前再次套用同一 public-output 边界。Admin UI 的紧凑状态标签会优先展示 title，再回退 URL，因此操作者可以区分 OpenCode、Google、callback 和空白页阶段，同时不暴露授权 payload。代码提交 `e6f4b229` 已覆盖 Node、Go、前端 helper、跨包 Go、typecheck、targeted oxlint 与 default 前端构建；已推送的 `main` 提交 `d6f31cbe` 随后通过远端 clean rollout verification-only 与完整 apply。 |
| Auth sidecar runtime smoke gate | 已实现并完成远端 apply | 已新增 `scripts/opencode-auth-session-smoke.mjs`。该无凭证 runtime smoke 会让 sidecar 打开 `about:blank`，验证 running status，发送白名单 Escape 控制键，验证 PNG 截图输出，停止会话并 purge 浏览器 artifact。报告只暴露固定状态类别和布尔检查，不打印浏览器 URL/title、截图字节、state 路径、账号材料或部署 endpoint。clean rollout helper 会安装该 runner，并在 Node gate 中覆盖它的测试与语法检查。代码提交 `7aa5d64f` 已通过本地完整 Node rollout gate，共 89 个子测试；提交 `3e030e09` 修复了 runner 的 symlinked artifact 入口，因为第一次 post-apply 检查发现已安装脚本可能退出 0 但没有执行 `main()`。已推送的 `main` 提交 `0dbd7964` 通过远端 verification-only 与完整 apply；apply 后独立检查确认已安装 runner 返回成功 JSON 摘要，start/status/press/screenshot/stop 检查全部为 true。提交 `c60a4dc0` 把该 smoke 变成 clean rollout 默认 apply gate，而不再依赖人工 post-apply 步骤；已推送的 `main` 提交 `243055ca` 验证完整 apply 路径现在会在 reported deployed prefix 前输出 `auth_runtime_smoke=ok`。 |
| Xvfb display 冲突重试 | 已实现并完成远端 apply | sidecar 不再把 account-derived Xvfb display 当作单点。Linux 且没有既有 `DISPLAY` 时，`login/start` 会从账号派生 display 开始尝试一组有界候选，只对可重试的 `Xvfb exited before ready` 启动失败切换到下一个 display，并仍清理每次失败尝试已经拉起的 browser/Xvfb 进程。该改动修复了真实远端 runtime 中 display 冲突导致的启动失败，同时不会掩盖缺少 browser 或 Xvfb 依赖这类硬故障。回归测试锁定候选序列，临时远端候选 smoke 已通过新重试路径；已推送的 `main` 提交 `0dbd7964` 已通过 clean rollout 与部署后的已安装 auth runtime smoke。 |
...
| 登录会话所有权保护 | 本地已实现 | login start/status/screenshot/click/key/press/stop controller action 现在会先确认持久 OpenCode account 行存在，再调用 sidecar。缺失账号 ID 会返回同一脱敏的账号不存在业务失败，并且不会创建或触碰浏览器 artifact。 |
...
| Codex OAuth 凭据提取 | 已实现并完成远端 apply | extractor 现在能从 storage 或同站 JSON responses 中识别完整 Codex OAuth credential object。只有当 `access_token` 同时带有 `account_id`，或能从 access-token JWT claim 推出 account ID 时，才会把已知字段序列化为现有 Codex channel JSON key 契约；单独的 OAuth token 字段仍会被拒绝为 API-key 候选。服务层测试覆盖提取、JWT 推导账号身份，以及用提取到的凭据激活绑定的 Codex channel。已推送的 `main` 提交 `98b804ee` 已在远端通过 verification-only gate、完整 apply 与 apply 后服务/runtime-script smoke。 |
...
| 前端账号窗口 | 已实现 | 已增加 Root-only 管理路由、侧边栏入口、账号列表、已启用 channel 选择器与数字 ID fallback、远端截图控制、安全控制键按钮、extract、quota refresh、activate、stop、delete 操作。账号列表现在使用固定的创建/绑定工具栏与独立滚动的 table 区域，因此导入多个 OpenCode 账号后不会把远端浏览器工作区推出固定管理后台表面。嵌入浏览器截图现在放在稳定的 button-backed click surface 中，使操作者点击稳定落到交互区域。 |
...
| 前端截图 viewport 契约 | 本地已实现 | sidecar 现在从 `Page.captureScreenshot` 输出解析 PNG width/height，并与 `image_base64` 一起返回；Go 截图契约保留这些尺寸；Admin Web 将截图保存为 `{ imageBase64, width, height }`，并按当前 bitmap 尺寸映射 pointer 坐标。缺失或非法尺寸会清空截图，而不是回退到陈旧的硬编码 viewport。这修复了已证明的坐标映射根因，但仍需完成远端 rollout，并由操作者执行真实 Google 登录尝试后，才能声称真实账号授权完成。如果后续 Google 因策略或反滥用机制阻挡嵌入式 CDP 交互，更好的兜底是基于同一远端 Chromium profile 的独立远端浏览器 viewer，而不是本地浏览器 cookie 提取。 |
| 远端浏览器弹出窗口 | 本地已实现 | Admin Web 可以把 `/opencode-browser?account_id=<id>` 打开为按当前屏幕尺寸约束的同源 popup。该 popup 仍按 super-admin route 守卫，但不经过普通 authenticated sidebar layout，因此 Google/OpenCode 交互获得独立 viewport。它复用同一组远端会话 API 与截图尺寸映射器，支持在 popup 内切换账号，并且 URL 状态只包含非 secret 的数字账号 ID。 | 这是当前最优先的 Google 交互兜底方案。它仍需远端 rollout 和真实操作者登录尝试。如果 full-window CDP viewer 仍不能满足 Google 交互要求，再考虑 noVNC/streaming 传输层，而不是本地浏览器 cookie 提取。 |
...
extractor 现在会区分不完整 OAuth 产物与完整 Codex 凭据。裸 `access_token` 仍不足以成为 API key；但满足 Codex channel 契约的对象，也就是同时包含 `access_token` 与 `account_id` 的对象，会被序列化为 Codex relay 已经消费的 JSON key 形态。如果对象缺少 `account_id`，extractor 可以通过既有 Codex OAuth 解析边界从 access-token JWT claim 推导该字段。这缩小了真实登录阶段的一个缺口：如果 OpenCode 页面暴露的是 Codex-compatible credential object，而不是 `api_key` 字段，账号仍可以进入 activation-ready 状态，同时不放宽 OAuth-token 误报防线。
...
最新 live E2E gate 消费这些类别，而不是重新实现 readiness。正确的所有权划分是：New API service/controller 决定账号是否可激活，preflight 把这个公开状态汇总成固定类别，live E2E 只在同一摘要证明至少一个 `ready` activation contract 时才允许进入 cache smoke。这个取舍故意偏严格：如果部署报告了 `active_ready`，但缺少 `activation_contract.ready`，现在会在消耗真实 `glm-5.2` quota 前失败；操作者应先修复账号、channel 或 Codex 凭据契约，或者只在生产验收之外显式使用诊断 override。

最新 preflight 加固把另一个假设前移到 API 边界：账号 item 字段必须具备 readiness accounting 所需的精确类型。这不是在可信内部代码里散落防御检查，而是在外部 HTTP payload 变成内部摘要前做边界验证。关键失败模式是 JavaScript truthiness：如果没有这个 gate，字符串 `active` 或非数组 `missing_activation_fields` 可能在后续 active-ready 与 activation-contract 检查前扭曲账号计数。新的 `opencode_accounts_item_contract` 会直接让这类 payload 失败，并且只报告固定字段类别计数。

最新 live E2E 加固把 acceptance 显式化，而不是要求操作者或自动化从 stage checks 推断。`checks.status` 仍然描述实际执行的 stage 是否通过，`acceptance.production_ready` 则描述这次运行是否可作为生产验收 gate。这里很关键：`--skip-tailscale true` 对诊断账号/cache 行为有价值，但它违背了 robust path 必须经过 Tailscale 的要求；同样，`--skip-auth-smoke true` 或非官方 auth smoke URL 也不能证明部署态浏览器桥能够访问官方 OpenCode 授权路径。取舍是 CLI 行为更严格：诊断运行即使 stage 自身通过，也可能非零退出。这是刻意设计；诊断 JSON 仍有用，但它不是可部署的验收信号。
...
```text
RED: bun test src/features/opencode-accounts/lib.test.ts
  因 shouldClearOpenCodeLoginScreenshotForStatus 缺失而失败
bun test src/features/opencode-accounts/lib.test.ts
  17 pass
bunx oxlint -c .oxlintrc.json src/features/opencode-accounts src/routes/_authenticated/opencode-accounts
  pass
...
apply 命令在返回完整 helper transcript 前被中断，因此本文档不声明该 revision 的 `deployed_prefix` 行。当前验收证据来自部署态 runtime：服务健康、已安装脚本语法、auth gate marker、脱敏 marker、官方 OpenCode auth-session smoke 成功和临时 helper 清理。

已推送提交 `24306450` 的最新远端 rollout 验证：
...
已推送提交 `300ea33d` 的最新远端 verification-only 验证：
...
已推送提交 `3404b01c` 的最新远端 rollout 验证：
...
已推送提交 `713392b0` 的此前运行时 rollout 验证：
...
远端 clean artifact 上线：
  ...
  apply 后独立 smoke 确认服务 active、本机 HTTP status OK、已安装 runtime scripts 语法 OK，auth sidecar 空状态 status OK
  clean rollout helper 针对已推送提交 16c126d1 的 verification-only gate 通过 git_clone/revision/node_scripts/go_targeted（包含跨包 Go gate）/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper 针对已推送提交 16c126d1 的远端完整 apply 通过 git_clone/revision/node_scripts/go_targeted（包含跨包 Go gate）/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke
  apply 后独立 smoke 确认服务 active、本机 HTTP status OK、已安装 runtime scripts 语法 OK，auth sidecar 空状态 status OK
  clean rollout helper 针对已推送提交 8c5d4e25 的 verification-only gate 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper 针对已推送提交 8c5d4e25 的远端完整 apply 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke/auth_runtime_smoke
  apply 后独立 smoke 确认服务 active、本机 HTTP status OK，auth sidecar、OpenCode preflight、cache smoke、Tailscale preflight、live E2E orchestrator 与 clean rollout helper 运行时脚本语法 OK
  本次 8c5d4e25 上线使用的临时 rollout helper 已在验证成功后清理
  clean rollout helper 针对已推送提交 98b804ee 的 verification-only gate 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper 针对已推送提交 98b804ee 的远端完整 apply 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke/auth_runtime_smoke
  apply 后独立 smoke 确认服务 active、本机 HTTP status OK，auth sidecar、OpenCode preflight、cache smoke、Tailscale preflight、live E2E orchestrator 与 clean rollout helper 运行时脚本语法 OK
  本次 98b804ee 上线使用的临时 rollout helper 已在验证成功后清理
  clean rollout helper 针对已推送提交 300ea33d 的 verification-only gate 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact，apply 已跳过，临时 helper 已清理
  clean rollout helper 针对已推送提交 24306450 的 verification-only gate 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact
  clean rollout helper 针对已推送提交 24306450 的远端完整 apply 通过 git_clone/revision/node_scripts/go_targeted/web_default_checks/web_default_build/web_classic_build/go_build/artifact/service_contract/backup/install/restart/http_smoke/auth_runtime_smoke
  apply 后独立 smoke 确认服务 active、本机 HTTP status OK、已安装 live E2E 脚本语法 OK、relaxed Tailscale acceptance marker 存在，且临时 helper 已清理
```
...
- stopped-session 截图生命周期修复证明 Admin Web 不会在 status 已报告 stopped 后继续把 stale bitmap 呈现成可交互浏览器。它不证明浏览器为什么停止；如果 Chromium 在真实 Google 登录期间退出，仍需继续做浏览器生命周期诊断。
...
- 提交 `300ea33d` 的 CLI 诊断退出回归测试使用 fake local New API server。它证明的是进程退出语义、summary 形态、cache stage 组合与 CLI 边界脱敏；它不是真实账号、真实 quota 或上游 cache 计费证据。
- 提交 `24306450` 的 relaxed-Tailscale acceptance 回归测试证明被放宽的 Tailscale gate 不会被误当成生产验收。它不证明当前 tailnet 已经 direct path；它只是防止诊断放宽项被升级成最终证据。
- 提交 `3404b01c` 的 relaxed OpenCode/cache evidence 回归测试证明被放宽的账号 readiness、credential key、affinity stats、模型、stats、请求次数、命中率或 cache-signal gate 不会被误当成生产验收。它不证明真实 OpenCode 订阅账号已经导入、激活，或已经拿到上游 warm-cache 计费收益。
- 提交 `0cdfdc74` 的官方 auth-session live E2E 回归测试证明生产验收现在会在 root-only OpenCode/cache gate 前要求无凭据官方 OpenCode 浏览器桥 stage。部署态 runtime 现在已经携带该 gate，并通过无凭据官方 OpenCode auth-session smoke。这仍不证明真实登录、账号提取、channel 激活或上游 cache 计费。
...
2. 在导入账号或执行真实 cache 测试前，先对当前部署态服务运行 preflight；clean rollout helper 继续作为后续已推送 revision 的可重复重同步路径。先用 verification-only，只有明确要替换运行 artifact 时才使用 `--apply true`：
