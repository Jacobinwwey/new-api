...
| Clean rollout runtime script set | Implemented locally | The rollout install contract now includes the operational scripts required after an artifact switch: `opencode-auth-session`, OpenCode preflight, cache smoke, Tailscale preflight, and the live E2E orchestrator. Regression coverage locks the exact set so future clean rollouts do not test scripts from the checkout while omitting their runtime copies. This is local hardening until a future pushed revision is applied on the remote host. |
...
| Clean rollout runtime script set | 本地已实现 | rollout 安装契约现在包含 artifact 切换后需要继续使用的操作脚本：`opencode-auth-session`、OpenCode preflight、cache smoke、Tailscale preflight 与 live E2E orchestrator。回归测试锁定完整集合，避免后续 clean rollout 只在 checkout 中测试脚本，却漏装运行目录副本。该项仍是本地加固，直到未来某个已推送 revision 在远端执行 apply。 |
