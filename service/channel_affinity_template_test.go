...
func TestChannelAffinityHitCodexTemplatePassHeadersEffective(t *testing.T) {
	...
	for _, modelName := range []string{"gpt-5", "glm-5.2"} {
		t.Run(modelName, func(t *testing.T) {
			affinityValue := fmt.Sprintf("pc-hit-%s-%d", strings.ReplaceAll(modelName, ".", "-"), time.Now().UnixNano())
			cacheKeySuffix := buildChannelAffinityCacheKeySuffix(*codexRule, modelName, "default", affinityValue)
			...
			channelID, found := GetPreferredChannelByAffinity(ctx, modelName, "default")
			require.True(t, found)
			...
			_, err := relaycommon.ApplyParamOverrideWithRelayInfo([]byte(fmt.Sprintf(`{"model":"%s"}`, modelName)), info)
			require.NoError(t, err)
